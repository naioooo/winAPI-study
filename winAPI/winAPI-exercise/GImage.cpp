...
HRESULT GImage::init(float x, float y, int width, int height, int maxFrameX, int maxFrameY, bool isTrans, COLORREF transColor)
{
    if (_imageInfo != nullptr)
    {
        this->release();
...
void GImage::frameRender(HDC hdc, int destX, int destY)
{
    if (_isTrans)
    {
        ...
        (
            hdc,
            destX, destY,                         // 시작점
            _imageInfo->frameWidth, _imageInfo->frameHeight, // 그릴 사이즈
            _imageInfo->hMemDC,                         // 복사될 대상 메모리 DC
            _imageInfo->currentFrameX * _imageInfo->frameWidth, _imageInfo->currentFrameY * _imageInfo->frameHeight, // 복사 시작지점
            _imageInfo->frameWidth, _imageInfo->frameHeight,  // 복사 사이즈
            _transColor                           // 제외할 색상
        );
...
{
    setFrameX(currentFrameX);
    setFrameY(currentFrameY);
